...
class LAEChain(chainer.Chain, AEBase):
    ...
    def __init__(self, in_size, out_size, activation=F.relu, batch_norm=False,
                 **kwargs):
        super().__init__()
...
class CAEChain(chainer.Chain, AEBase):
    ...
    def __init__(self, in_channels, out_channels, ksize=3, padding=False,
                 activation=F.relu, use_indices=False, batch_norm=False, **kwargs):
        super().__init__()
    ...
    def encode(self, x, **kwargs):
        # print(f'E{self.name}:', F.min(x), F.max(x))
        h = self.enc(x)
        if self.activation_e:
            if self.batch_norm:
                h = self.bne(h)
